image:doc/img/logo.png[doc/img/logo]

image:https://img.shields.io/github/v/tag/cybergarage/puzzledb-go[GitHub
tag (latest SemVer)]
https://github.com/cybergarage/puzzledb-go/actions/workflows/make.yml[image:https://github.com/cybergarage/puzzledb-go/actions/workflows/make.yml/badge.svg[Go]]
https://pkg.go.dev/github.com/cybergarage/puzzledb-go[image:https://pkg.go.dev/badge/github.com/cybergarage/puzzledb-go.svg[Go
Reference]]

PuzzleDB aims to be a high-performance, distributed, cloud-native, multi-API, multi-model database. This Technology Preview version is developed in the Go language.

include::doc/whatis.adoc[]

= Get Started

See the following guide to learn about how to get started.

* link:doc/quick-start.md[Quick Start]

= How does PuzzleDB work?

For architecture of PuzzleDB, see the following concept documents:

* link:doc/concept.md[Design Concepts]
** link:doc/layer-concept.md[Layer Concept]
*** link:doc/plugin-concept.md[Plug-In Concept]
** link:doc/coordinator-concept.md[Coordinator Concept]
** link:doc/storage-concept.md[Storage Concept]
** link:doc/data-model.md[Data Model]
** link:doc/consistency-model.md[Consistency Model]

= Supported Protocols

This technology preview version partially supports major database models and protocols. Please refer to the following documents for details on support status and limitations.

* Compatibility
** link:doc/redis.md[Redis]
** link:doc/mongodb.md[MongoDB]
** link:doc/mysql.md[MySQL]
** PostgreSQL (Planning)

= Roadmap

PuzzleDB is currently in a technical preview release. Currently,
PuzzleDB is in the process of developing and testing a distributed
plugin for the next release. The development roadmap for PuzzleDB is shown below.

image:doc/img/roadmap.png[doc/img/roadmap]

For more information about the roadmap, please refer to link:doc/roadmap.adoc[Roadmap].

= User Guides

* Get Started
** link:doc/quick-start.md[Quick Start]
*** link:doc/cmd/server/puzzledb-server.md[puzzledb-server]
** Docker Hub
*** link:https://hub.docker.com/repository/docker/cybergarage/puzzledb/general[cybergarage/puzzledb]
* Configuration
** link:doc/configuring.md[Configuring PullzeDB]
* Operation
** CLI
*** link:doc/cmd/cli/puzzledb-cli.md[puzzledb-cli]
** link:doc/tracing.md[Tracing]

= Developer Guides

* References
** link:https://pkg.go.dev/github.com/cybergarage/puzzledb-go[Go Reference]
* Contributing (Planning)
** link:doc/coding_guideline.md[Coding Guidelines]
* Building and Testing
** link:doc/build-on-macos.md[Build on macOS]
** link:doc/build-on-macos.md[Build on Ubuntu]
* Extending PuzzleDB
** link:doc/plugin-concept.md[Plug-In Concept]
** link:doc/plugin-types.md[Plug-in Services]
*** link:doc/plugin-tutorial.md[Building User Plug-ins]
* Specification
** link:doc/storage-kv.md[Key-Value Store Specification]