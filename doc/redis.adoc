...
The https://github.com/cybergarage/go-redis[go-redis] framework automatically handles the Redis protocol and system commands such as PING and SELECT. Therefore, PuzzleDB achieves compatibility with Redis by implementing only user commands such as SET and GET.
